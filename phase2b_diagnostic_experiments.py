...
RECENT FIXES & ENHANCEMENTS (addressing critical function signature issues):
- Fixed train_with_film_awareness call signature (experiment_name first!)
- ENABLED FiLM regularization by default (use_film_reg=True)
- Flexible film_generator_type detection (infers from checkpoint path)
- Full config passed to train_with_film_awareness (warmup, clipping, etc.)
- Enhanced data loader detection (multiple naming conventions)
- Diagnostic quality tracking (full vs minimal signature usage)
- Comprehensive error handling and fallback mechanisms
- All 4 hypotheses now use proper FiLM-aware training dynamics
"""
...
def run_corrected_diagnostic_experiment(experiment_config: Dict, 
                                       num_epochs: int = 5,
                                       base_checkpoint_path: str = "/content/checkpoints/Phase2a_Only_PhaseA_FrontendFrozen_best_mape (1).pth") -> Dict:
    """
    ENHANCED notebook integration function with full FiLM-aware diagnostics.
    
    This function properly enables FiLM regularization, handles flexible model architecture,
    and uses the full-featured train_with_film_awareness function for accurate diagnostics.
    
    Key Improvements:
    - FiLM regularization enabled by default (use_film_reg=True)
    - Flexible film_generator_type (inferred or specified)
    - Full config passed to train_with_film_awareness
    - Better error handling and validation
    """
    experiment_id = experiment_config.get('experiment_id', 'corrected_test')
    ...
    try:
        # Check if notebook functions are available
        import inspect
        
        # Try to get existing functions from notebook globals
        ...
        for func_name in required_functions:
            if func_name not in notebook_globals:
        ...
        if missing_functions:
            ...
            return {
                'experiment_id': experiment_id,
                'success': False,
                'error': f"Missing required functions: {missing_functions}",
                'mape': float('inf')
            }
        
        print("✅ All existing functions found - proceeding with reuse")
        
        # Get the classes from notebook
        CompleteSincGAT_UNet = notebook_globals['CompleteSincGAT_UNet']
        RefinedLogSpaceMAEHybridLoss = notebook_globals['RefinedLogSpaceMAEHybridLoss']
        train_with_film_awareness = notebook_globals['train_with_film_awareness']
        calculate_mape = notebook_globals['calculate_mape']
        
        # Determine film_generator_type - flexible approach
        film_generator_type = experiment_config.get('film_generator_type', None)
        if film_generator_type is None:
            # Infer from checkpoint path or use default
            if 'linear' in base_checkpoint_path.lower() or 'Linear' in base_checkpoint_path:
                film_generator_type = 'linear'
            else:
                film_generator_type = '2_layer'  # Default for most Phase 2a experiments
        
        print(f"🔄 Loading base model with {film_generator_type} FiLM generator...")
        print(f"📁 Checkpoint: {base_checkpoint_path}")
        
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = CompleteSincGAT_UNet(film_generator_mlp_type=film_generator_type).to(device)
        ...
        print(f"✅ Model created with 2_layer FiLM generator")
        print(f"✅ Loaded checkpoint with Phase 2a MAPE: {base_mape}")
        
        # Handle U-Net freezing/unfreezing
        if experiment_config.get('keep_unet_frozen', False):
            for param in model.unet.parameters():
                param.requires_grad = False
        ...
        # Create loss criterion using existing class with FULL FiLM awareness
        use_film_reg = experiment_config.get('use_film_reg', True)  # ENABLED by default for diagnostics
        lambda_gamma_res = experiment_config.get('lambda_gamma_res', 0.005)
        lambda_beta_res = experiment_config.get('lambda_beta_res', 0.0005)
        
        print("🔧 Using existing RefinedLogSpaceMAEHybridLoss class...")
        print(f"   🎯 FiLM regularization: ENABLED ({use_film_reg})")
        print(f"   📊 λ_gamma_res: {lambda_gamma_res}, λ_beta_res: {lambda_beta_res}")
        ...
        criterion = RefinedLogSpaceMAEHybridLoss(
            ...
            start_simple=True,
            curriculum_epochs=2,
            use_film_reg=use_film_reg,  # ENABLED for proper FiLM diagnostics
            lambda_gamma_res=lambda_gamma_res,
            lambda_beta_res=lambda_beta_res
        ).to(device)
        print("✅ Loss criterion created with FiLM regularization enabled")
        
        # Create optimizer using existing function (if available) or ENHANCED fallback
        print("🔧 Creating Phase 2b optimizer with differential learning rates...")
        
        create_phase2_optimizer_groups = notebook_globals.get('create_phase2_optimizer_groups')
        if create_phase2_optimizer_groups:
            print("✅ Found existing create_phase2_optimizer_groups function")
            optimizer = create_phase2_optimizer_groups(
                model=model,
                lr_frontend=experiment_config.get('lr_frontend', 5e-5),
                lr_unet=experiment_config.get('lr_unet', 1e-5),
                lr_film_generator=experiment_config.get('lr_film_generator', 1e-4),
                weight_decay=0.01
            )
            print("✅ Differential learning rate optimizer created successfully")
        else:
            print("⚠️ create_phase2_optimizer_groups not found - using ENHANCED fallback")
            print("🔧 Creating manual differential learning rate groups...")
            
            # ENHANCED fallback with differential learning rates
            lr_frontend = experiment_config.get('lr_frontend', 5e-5)
            lr_unet = experiment_config.get('lr_unet', 1e-5) 
            lr_film_generator = experiment_config.get('lr_film_generator', 1e-4)
            weight_decay = 0.01
            ...
            sincnet_params = []
            if hasattr(model, 'shot_encoder'):
                sincnet_params = list(model.shot_encoder.parameters())
            ...
            gat_norm_params = []
            if hasattr(model, 'gat_context_layernorm'):
                gat_norm_params = list(model.gat_context_layernorm.parameters())
            ...
            if film_params:
                param_groups.append({
                    'params': film_params,
                    'lr': lr_film_generator,
                    'weight_decay': 1e-3,  # Lower weight decay for FiLM
                    'group_name': 'FiLM'
                })
            ...
            if not param_groups:
                ...
                param_groups = [{'params': model.parameters(), 'lr': lr_unet, 'weight_decay': weight_decay}]
                
            optimizer = torch.optim.AdamW(param_groups)
            
            print(f"📊 ENHANCED fallback optimizer created:")
            for i, group in enumerate(param_groups):
                group_name = group.get('group_name', f'Group_{i}')
                lr = group['lr']
                param_count = sum(p.numel() for p in group['params'])
                print(f"   {group_name}: lr={lr:.1e}, params={param_count:,}")
            
            print("✅ Differential learning rate optimizer created successfully")
        
        # Get data loaders from notebook globals - flexible naming
        print("📊 Using EXISTING data loaders from notebook...")
        
        # Try multiple possible data loader names
        possible_train_names = ['train_loader', 'train_loader_phase2', 'phase2_train_loader']
        possible_val_names = ['val_loader', 'val_loader_phase2', 'phase2_val_loader']
        ...
        for name in possible_train_names:
            if name in notebook_globals:
                train_loader = notebook_globals[name]
                print(f"   📚 Found train loader: {name}")
                break
                
        for name in possible_val_names:
            if name in notebook_globals:
                val_loader = notebook_globals[name]
                print(f"   📊 Found val loader: {name}")
                break
        
        if train_loader is None or val_loader is None:
            available_loaders = [k for k in notebook_globals.keys() if 'loader' in k.lower()]
            print(f"❌ Data loaders not found in notebook globals")
            print(f"   Available loader-like variables: {available_loaders}")
            return {
                'experiment_id': experiment_id,
                'success': False,
                'error': f"Data loaders not available. Found: {available_loaders}",
                'mape': float('inf')
            }
        ...
        if scheduler_type == 'ReduceLROnPlateau':
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=num_epochs, eta_min=1e-7, verbose=True
            )
        ...
        # Get the train_with_film_awareness function signature for diagnostics
        sig = inspect.signature(train_with_film_awareness)
        print(f"🔍 train_with_film_awareness signature: {sig}")
        ...
        has_config = 'config' in available_params
        has_lr_scheduler = 'lr_scheduler' in available_params  
        has_checkpoint_freq = 'checkpoint_freq' in available_params
        print(f"🔍 Parameter availability: config={has_config}, lr_scheduler={has_lr_scheduler}, checkpoint_freq={has_checkpoint_freq}")
        
        # Prepare FULL FiLM-aware config for diagnostic training
        full_config = {
            'warmup_steps': experiment_config.get('warmup_steps', 200),  # Modest warmup for frontend
            'gradient_clip_film': experiment_config.get('gradient_clip_film', 1.0),
            'gradient_clip_others': experiment_config.get('gradient_clip_others', 5.0),
            'use_grad_clipping': experiment_config.get('use_grad_clipping', True),
            'use_film_reg': use_film_reg,  # ENABLED for proper FiLM diagnostics
            'monitor_freq': experiment_config.get('monitor_freq', 50),
            'epoch_monitor_freq': experiment_config.get('epoch_monitor_freq', 5)
        }
        
        print(f"🎯 Full FiLM config: {full_config}")
        
        # CRITICAL DEBUG: Verify argument types before function call
        print(f"🔍 DEBUG - Argument types before function call:")
        print(f"   experiment_id: {type(experiment_id)} = '{experiment_id}'")
        print(f"   model: {type(model)} = {model.__class__.__name__}")
        print(f"   train_loader: {type(train_loader)} = {train_loader.__class__.__name__}")
        print(f"   val_loader: {type(val_loader)} = {val_loader.__class__.__name__}")
        print(f"   criterion: {type(criterion)} = {criterion.__class__.__name__}")
        print(f"   optimizer: {type(optimizer)} = {optimizer.__class__.__name__}")
        print(f"   num_epochs: {type(num_epochs)} = {num_epochs}")
        print(f"   device: {type(device)} = {device}")
        print(f"   calculate_mape: {type(calculate_mape)} = {calculate_mape.__name__}")
        
        # CRITICAL SAFETY CHECK: Ensure model is actually a model, not a DataLoader
        if hasattr(model, '__len__') and not hasattr(model, 'parameters'):
            print(f"🚨 CRITICAL ERROR: model argument appears to be a DataLoader, not a PyTorch model!")
            print(f"   model type: {type(model)}")
            print(f"   model has __len__: {hasattr(model, '__len__')}")
            print(f"   model has parameters: {hasattr(model, 'parameters')}")
            raise ValueError("Model argument is not a PyTorch model - argument order may be wrong!")
        
        # Call train_with_film_awareness with correct signature (experiment_name first!)
        try:
            # First try with the FULL signature for maximum FiLM awareness
            print("🎬 Attempting FULL train_with_film_awareness call...")
            training_results = train_with_film_awareness(
                ...
                lr_scheduler=scheduler,  # FIXED: was 'scheduler', now 'lr_scheduler'
                config=full_config,
                checkpoint_freq=10  # RESTORED: this parameter is supported
            )
            print("✅ FULL train_with_film_awareness call successful!")
            
        except TypeError as e:
            print(f"⚠️ Full signature failed: {e}")
            if "unexpected keyword argument" in str(e):
                print("🔄 Falling back to MINIMAL signature...")
                print(f"🔍 Attempting minimal call with args: experiment_id='{experiment_id}', model=<{type(model).__name__}>, ...")
                
                # EXTRA SAFETY: Double-check argument types for fallback call
                print(f"🔍 FALLBACK DEBUG - Final argument verification:")
                print(f"   Arg 1 (experiment_name): {type(experiment_id)} = '{experiment_id}'")
                print(f"   Arg 2 (model): {type(model)} = {model.__class__.__name__}")
                print(f"   Arg 3 (train_loader): {type(train_loader)} = {train_loader.__class__.__name__}")
                print(f"   Arg 4 (val_loader): {type(val_loader)} = {val_loader.__class__.__name__}")
                print(f"   Arg 5 (criterion): {type(criterion)} = {criterion.__class__.__name__}")
                print(f"   Arg 6 (optimizer): {type(optimizer)} = {optimizer.__class__.__name__}")
                print(f"   Arg 7 (num_epochs): {type(num_epochs)} = {num_epochs}")
                print(f"   Arg 8 (device): {type(device)} = {device}")
                print(f"   Arg 9 (calculate_mape_func): {type(calculate_mape)} = {calculate_mape.__name__}")
                
                # FINAL CORRECTED: Positional-only call with EXACT argument order
                training_results = train_with_film_awareness(
                    experiment_id,              # 1st: experiment_name ✅
                    model,                      # 2nd: model ✅ 
                    train_loader,               # 3rd: train_loader ✅
                    val_loader,                 # 4th: val_loader ✅
                    criterion,                  # 5th: criterion ✅
                    optimizer,                  # 6th: optimizer ✅
                    num_epochs,                 # 7th: num_epochs ✅
                    device,                     # 8th: device ✅
                    calculate_mape              # 9th: calculate_mape_func ✅
                )
                print("⚠️ WARNING: Using minimal signature - reduced FiLM diagnostics fidelity")
                print("✅ Minimal call completed successfully")
            else:
        ...
        if isinstance(training_results, tuple):
            best_mape = training_results[0]
        elif isinstance(training_results, dict):
            best_mape = training_results.get('best_val_mape', float('inf'))
        else:
            best_mape = training_results
            
        print(f"✅ Training completed - Best MAPE: {best_mape:.6f}%")
        ...
        return {
            ...
            'best_mape': best_mape,
            'success': True,
            'film_reg_enabled': use_film_reg,
            'film_generator_type': film_generator_type,
            'full_signature_used': 'FULL train_with_film_awareness call successful!' in locals(),
            'timestamp': datetime.now().isoformat()
        }
...
def run_corrected_quick_diagnostic_test():
    """
    ENHANCED FiLM-aware diagnostic test for notebook integration.
    
    This version:
    - ENABLES FiLM regularization by default (use_film_reg=True)
    - Uses flexible film_generator_type detection
    - Passes full config to train_with_film_awareness
    - Tests all 4 systematic hypotheses with proper FiLM dynamics
    """
    print("🧪 Starting ENHANCED FiLM-Aware Quick Diagnostic Test")
    print("=" * 70)
    print("✅ Using train_with_film_awareness (proven infrastructure)")
    print("✅ FULL FiLM regularization + warmup + differential clipping")
    print("✅ Flexible architecture detection + enhanced error handling")
    print("=" * 70)
    
    # Define the 4 ENHANCED diagnostic hypotheses with full FiLM awareness
    diagnostic_experiments = [
        {
            'experiment_id': 'hypothesis_1_ultra_low_unet_lr',
            'lr_unet': 2e-6,  # Ultra-low U-Net LR (key hypothesis)
            'lr_frontend': 5e-5,
            'lr_film_generator': 1e-4,
            'lambda_gamma_res': 0.005,
            'lambda_beta_res': 0.0005,
            'scheduler_type': 'ReduceLROnPlateau',
            'use_film_reg': True,  # ENABLED for proper diagnostics
            'film_generator_type': '2_layer',  # Explicit for base checkpoint
            'warmup_steps': 200,  # Frontend warmup
            'gradient_clip_film': 1.0,
            'gradient_clip_others': 5.0
        },
        {
            'experiment_id': 'hypothesis_2_higher_film_reg',
            'lr_unet': 1e-5,
            'lr_frontend': 5e-5,
            'lr_film_generator': 1e-4,
            'lambda_gamma_res': 0.01,  # HIGHER FiLM regularization (key hypothesis)
            'lambda_beta_res': 0.001,  # HIGHER beta regularization
            'scheduler_type': 'ReduceLROnPlateau',
            'use_film_reg': True,  # ENABLED
            'film_generator_type': '2_layer',
            'warmup_steps': 200,
            'gradient_clip_film': 0.5,  # Tighter FiLM clipping for strong reg
            'gradient_clip_others': 5.0
        },
        {
            'experiment_id': 'hypothesis_3_cosine_scheduler',
            'lr_unet': 1e-5,
            'lr_frontend': 5e-5,
            'lr_film_generator': 1e-4,
            'lambda_gamma_res': 0.005,
            'lambda_beta_res': 0.0005,
            'scheduler_type': 'CosineAnnealingLR',  # COSINE scheduler (key hypothesis)
            'use_film_reg': True,  # ENABLED
            'film_generator_type': '2_layer',
            'warmup_steps': 300,  # Longer warmup for cosine
            'gradient_clip_film': 1.0,
            'gradient_clip_others': 5.0
        },
        {
            'experiment_id': 'hypothesis_4_extended_phase2a',
            'lr_frontend': 5e-5,
            'lr_film_generator': 1e-4,
            'lambda_gamma_res': 0.005,
            'lambda_beta_res': 0.0005,
            'keep_unet_frozen': True,  # EXTENDED Phase 2a (key hypothesis)
            'scheduler_type': 'ReduceLROnPlateau',
            'use_film_reg': True,  # ENABLED
            'film_generator_type': '2_layer',
            'warmup_steps': 500,  # Longer warmup for extended training
            'gradient_clip_film': 1.0,
            'gradient_clip_others': 5.0
        }
    ]
    
    print(f"\n🚀 Running {len(diagnostic_experiments)} diagnostic experiments...")
    print("⏱️ 5 epochs each for rapid feedback")
    
    results = []
    successful_experiments = 0
    
    for config in diagnostic_experiments:
        print(f"\n{'='*40}")
        print(f"🔬 Testing: {config['experiment_id']}")
        print(f"{'='*40}")
        ...
        if result.get('success', False):
            ...
            print(f"✅ Success: MAPE = {result['best_mape']:.6f}%")
        else:
    ...
    print(f"{'='*60}")
    print(f"✅ Successful experiments: {successful_experiments}/{len(diagnostic_experiments)}")
    
    if successful_experiments > 0:
        successful_results = [r for r in results if r.get('success', False)]
        best_result = min(successful_results, key=lambda x: x['best_mape'])
        print(f"🏆 Best MAPE: {best_result['best_mape']:.6f}% ({best_result['experiment_id']})")
        
        # Diagnostic quality assessment
        full_sig_count = sum(1 for r in successful_results if r.get('full_signature_used', False))
        film_enabled_count = sum(1 for r in successful_results if r.get('film_reg_enabled', False))
        
        print(f"🔬 Diagnostic Quality:")
        print(f"   Full train_with_film_awareness signature: {full_sig_count}/{successful_experiments}")
        print(f"   FiLM regularization enabled: {film_enabled_count}/{successful_experiments}")
        
        if full_sig_count == successful_experiments and film_enabled_count == successful_experiments:
            print("✅ HIGH FIDELITY diagnostics - all experiments used full FiLM awareness")
        elif full_sig_count > 0:
            print("⚠️ MIXED FIDELITY diagnostics - some experiments used reduced features")
        else:
            print("❌ LOW FIDELITY diagnostics - minimal training features used")
        
        # Check for breakthrough
        if best_result['best_mape'] < 0.0893:
            improvement = 0.0893 - best_result['best_mape']
            print(f"🎉 BREAKTHROUGH! Achieved {improvement:.6f}% improvement!")
            print(f"   Best config: {best_result['experiment_id']}")
        else:
            deficit = best_result['best_mape'] - 0.0893
            print(f"⚠️ Target not reached: {deficit:.6f}% above 0.0893% baseline")
            print(f"   Recommendation: Try focused experiments on {best_result['experiment_id']}")
    else:
        print("❌ No successful experiments")
    
    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_filename = f"phase2b_corrected_diagnostics"
...
def validate_notebook_integration():
    """
    Quick validation function to check notebook integration readiness.
    
    This function verifies that all required global functions and variables
    are available before running the full diagnostic suite.
    """
    ...
    # Check optional but recommended functions
    optional_functions = ['create_phase2_optimizer_groups']
    for func_name in optional_functions:
        if func_name in globals():
            print(f"✅ {func_name}: Available (recommended)")
        else:
            print(f"⚠️  {func_name}: Missing (will use enhanced fallback)")
    
    # Check data loaders
    possible_train_names = ['train_loader', 'train_loader_phase2', 'phase2_train_loader']
    possible_val_names = ['val_loader', 'val_loader_phase2', 'phase2_val_loader']
    
    train_loader_found = False
    val_loader_found = False
    
    for name in possible_train_names:
        if name in globals():
            print(f"✅ Train loader: {name} found")
            train_loader_found = True
            break
    
    for name in possible_val_names:
        if name in globals():
            print(f"✅ Val loader: {name} found")
            val_loader_found = True
            break
    
    if not train_loader_found:
        print("❌ Train loader: Not found")
    if not val_loader_found:
        print("❌ Val loader: Not found")
    
    # Check train_with_film_awareness signature
    if 'train_with_film_awareness' in globals():
        import inspect
        sig = inspect.signature(globals()['train_with_film_awareness'])
        params = list(sig.parameters.keys())
        print(f"🔍 train_with_film_awareness signature: {params}")
        
        has_config = 'config' in params
        has_lr_scheduler = 'lr_scheduler' in params
        print(f"   🎯 FiLM-aware features: config={has_config}, lr_scheduler={has_lr_scheduler}")
    
    # Summary
    print(f"\n📊 INTEGRATION SUMMARY")
    print(f"   Required functions: {len(available_functions)}/{len(required_functions)}")
    print(f"   Data loaders: train={train_loader_found}, val={val_loader_found}")
    
    ready = (len(missing_functions) == 0 and train_loader_found and val_loader_found)
    
    if ready:
        print("✅ READY FOR DIAGNOSTICS")
        print("🚀 You can now run: run_corrected_quick_diagnostic_test()")
        return {
            'all_dependencies_available': True,
            'missing_items': [],
            'ready': True,
            'missing_functions': [],
            'train_loader_found': train_loader_found,
            'val_loader_found': val_loader_found
        }
    else:
        print("❌ NOT READY - Missing components")
        missing_items = missing_functions[:]
        if not train_loader_found:
            missing_items.append('train_loader')
        if not val_loader_found:
            missing_items.append('val_loader')
            
        if missing_functions:
            print(f"   Missing functions: {missing_functions}")
        if not train_loader_found or not val_loader_found:
            print(f"   Missing data loaders")
        print("💡 Run missing cells in your notebook first")
